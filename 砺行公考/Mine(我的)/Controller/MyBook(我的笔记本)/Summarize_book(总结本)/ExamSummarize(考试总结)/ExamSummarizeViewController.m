...
@interface ExamSummarizeViewController ()<UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, QBImagePickerControllerDelegate, UITextViewDelegate, UITextFieldDelegate>

//标题
@property (nonatomic, strong) NSString *title_string;

//考试时间
@property (nonatomic, strong) NSString *tptime_string;
//考试状态
...
@implementation ExamSummarizeViewController
...
- (void)viewDidLoad {
    ...
    NSLog(@"exam image count === %ld", self.imageArray.count);
    
    [self setBack];
    self.title = @"考试总结";
    
    self.tptime_string = @"考试时间";
...
- (void)setHeader_view:(UICollectionReusableView *)header_view {
    ...
    NSArray *title_array = @[self.tptime_string, self.tpstatus_string, self.tpget_string, self.tpconfuse_string, self.tpsolve_string, self.tpremind_string];
    CGFloat height = 60.0;
...
- (void)textViewDidEndEditing:(UITextView *)textView {
    switch (textView.tag - 100) {
        case 0:
            self.tptime_string = textView.text;
            break;
        case 1:
...
- (void)finish_action {
    ...
    if (self.exam_id) {
        __weak typeof(self) weakSelf = self;
        NSDictionary *parma = @{@"id_":self.exam_id};
...
- (void)submitDataToService {
    if ([self.title_string isEqualToString:@""] &&
        [self.tptime_string isEqualToString:@"考试时间"] &&
    ...
    NSDictionary *parma = @{
                            ...
                            @"tp_time_":[self.tptime_string isEqualToString:@"考试时间"] ? @"" : self.tptime_string,
                            ...
                            @"tpvsolve_":[self.tpsolve_string isEqualToString:@"解决方案"] ? @"" : self.tpsolve_string,
                            @"tp_remind_":[self.tpremind_string isEqualToString:@"自我提醒"] ? @"" : self.tpremind_string,
                            @"type_":@"2",
                            };
    
    NSLog(@"kaoshi parma == %@", parma);
...
- (void)backSummarizeBookHomeVC {
    for (UIViewController *control in self.navigationController.viewControllers) {
        if ([control isKindOfClass:[SummarizeBookViewController class]]) {
            ...
            UIButton *button = (UIButton *)[summarize.view viewWithTag:101];
...
- (void)getHttpDataWithID {
    ...
    [MOLoadHTTPManager PostHttpDataWithUrlStr:@"/app_user/ass/find_total_detail" Dic:parma SuccessBlock:^(id responseObject) {
        ...
        if ([responseObject[@"state"] integerValue] == 1) {
            ...
            weakSelf.tptime_string = responseObject[@"data"][@"total_content_"][0][@"tp_time_"];
