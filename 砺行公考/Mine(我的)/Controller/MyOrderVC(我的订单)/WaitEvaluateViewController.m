...
@interface WaitEvaluateViewController ()<UITableViewDelegate, UITableViewDataSource, MyOrderFooterViewDelegate>
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    MyOrderModel *model = self.dataArray[section];
    footer_view.shop_finish_content.text = [NSString stringWithFormat:@"共%@件商品 合计：%@（含运费：%@）", model.order_commodity_total, model.order_real_payment, model.order_send_cost];
    return footer_view;
}
...
- (void)touchButtonTargetAction:(NSString *)buttonTitle {
    if ([buttonTitle isEqualToString:@"评价"]) {
