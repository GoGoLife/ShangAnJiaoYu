//
//  BigTrainingTenthViewController.m
//  砺行公考
//
...
@interface BigTrainingTenthViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation BigTrainingTenthViewController
...
- (void)getImage {
    ...
    [MOLoadHTTPManager PostHttpDataWithUrlStr:@"/app_user/course/five_training/find_logical_tree_diagram" Dic:param SuccessBlock:^(id responseObject) {
        if ([responseObject[@"state"] integerValue] == 1) {
            NSMutableArray *imageArr = [NSMutableArray arrayWithCapacity:1];
            for (NSDictionary *dic in responseObject[@"data"]) {
                [imageArr addObject:dic[@"path_"]];
            }
            weakSelf.image_array = [imageArr copy];
...
- (void)viewDidLoad {
    ...
    [self setleftOrRight:@"right" BarButtonItemWithTitle:@"下一步" target:self action:@selector(puchEleventhVC)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (tableView.tag == 2000) {
        if (section >= 2 && section <= self.fenxi_data_count + 1) {
            ...
            [self setRightTableviewFirstHeaderView:header_view withImageArray:self.image_array];
        }
    }
    return header_view;
}

- (void)setRightTableviewFirstHeaderView:(UIView *)header_view withImageArray:(NSArray *)imageArray {
    CGFloat width = (SCREENBOUNDS.width - 40) / 10 * 9;
    ...
    [scroll setContentSize:CGSizeMake(width * imageArray.count, 300.0)];
    [header_view addSubview:scroll];
    ...
    for (int index = 0; index < imageArray.count; index++) {
        ...
        [imageview sd_setImageWithURL:[NSURL URLWithString:imageArray[index]] placeholderImage:[UIImage imageNamed:@"no_image"]];
        [scroll addSubview:imageview];
        [imageview mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(scroll.mas_top);
            make.left.equalTo(scroll.mas_left).offset(width * index);
...
- (void)puchEleventhVC {
    BigTrainingEleventhViewController *eleventh = [[BigTrainingEleventhViewController alloc] init];
