//
//  BigTrainingFourthViewController.m
//  砺行公考
//
...
@interface BigTrainingFourthViewController ()

@property (nonatomic, strong) UITextView *yinyan_textview;

@end

@implementation BigTrainingFourthViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
//    self.title = @"第三步：布局总括--引言";
    [self setBack];
    ...
    self.yinyan_textview = [[UITextView alloc] init];
    self.yinyan_textview.font = SetFont(14);
    self.yinyan_textview.scrollEnabled = NO;
    self.yinyan_textview.backgroundColor = SetColor(246, 246, 246, 1);
    self.yinyan_textview.textContainerInset = UIEdgeInsetsMake(20, 20, 20, 20);
    ViewRadius(self.yinyan_textview, 8.0);
    self.yinyan_textview.text = @"在这里，魔法不只是一种神秘莫测的能量概念。它是实体化的物质，可以被引导、成形、塑造和操作。符文之地的魔法拥有自己的自然法则。源生态魔法随机变化的结果改变了科学法则。符文之地有数块大陆，不过所有的生命都集中在最大魔法大陆——瓦罗兰。瓦罗兰大陆居于符文之地中心它是实体化的物质，可以被引导、成形、塑造和操作。符文之地的魔法拥有自己的自然法则。源生态魔法随机变化的结果改变了科学法则。符文之地有数块大陆，不过所有的生命都集中在最大魔法大陆——瓦罗兰。瓦罗兰大陆居于符文之地中心 ";
    [self.view addSubview:self.yinyan_textview];
    [self.yinyan_textview mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)pushFifthVC {
    [self.yinyan_textview.text writeToFile:BigTraining_YinYan_File_Data atomically:YES encoding:NSUTF8StringEncoding error:nil];
    BigTrainingFifthViewController *fifth = [[BigTrainingFifthViewController alloc] init];
