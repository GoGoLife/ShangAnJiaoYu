//
//  BigTrainingFirstViewController.m
//  砺行公考
//
...
@interface BigTrainingFirstViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation BigTrainingFirstViewController

- (void)getData {
    __weak typeof(self) weakSelf = self;
    NSDictionary *param = @{@"exam_id_":self.bigTestTraining_id,@"order_":@"1"};
    [MOLoadHTTPManager PostHttpDataWithUrlStr:@"/app_user/course/five_training/find_first_step" Dic:param SuccessBlock:^(id responseObject) {
        ...
        if ([responseObject[@"state"] integerValue] == 1) {
            ...
            for (NSDictionary *dataDic in responseObject[@"data"]) {
                ...
                model.judge_parsing = [dataDic[@"judge"] count] == 0 ? @"" : dataDic[@"judge"][@"parsing"];
...
- (void)viewDidLoad {
    ...
    self.isShowParsing = NO;
    self.title = @"第一步：审题判断";
    [self setBack];
    
    [[NSUserDefaults standardUserDefaults] setObject:self.bigTestTraining_id forKey:@"bigTestTrainingExamID"];
...
- (void)pushSecondVC {
    TestTrainingFirstModel *model = self.dataArr[0];
    BigTrainingSecondViewController *second = [[BigTrainingSecondViewController alloc] init];
    second.model = model;
